...
- Run ```bash unlink /etc/nginx/sites-enabled/default #This will help unlink the default nginx configuration```
- Navigate to `/etc/nginx/conf.d/`
- Create a new file named `nodeapp.conf`
- Maintain the configuration for reverse proxy as seen below
  ```bash
         upstream nodeapp {
